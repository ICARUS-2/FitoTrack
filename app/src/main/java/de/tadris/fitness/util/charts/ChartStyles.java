...
public class ChartStyles {
    ...
    public static void setupPBLegend(Chart chart, double yMax, String unit, String type) {
        //creation a sorting of Legend for PBs
        ...
        LegendEntry distanceEntry = new LegendEntry();
        distanceEntry.formColor = Color.BLUE;
        String PB = "N/A";
        try{
            PB = "" + yMax/*personalBestRecords.get(0).getWorkout().length*/;
            PB = String.format("%.2f", yMax);

        }catch(Exception e){

        }

        distanceEntry.label = "PB " + type + ": " + PB + " " + unit; //+ chart.getData().getYMax();
