...
import static de.tadris.fitness.util.charts.ChartStyles.setupPBLegend;
...
public class StatsHistoryFragment extends StatsFragment {
    ...
    private void updateSpeedChart(List<WorkoutType> workoutTypes) {
        ...
        try {
            ...
            // Find the Highest Speed Data Point on Chart
            double highestSpeed = 0;
            int highestSpeedIndex = -1;
            for (int i = 0; i < candleDataSet.getEntryCount(); i++) {
                CandleEntry dataPoint = candleDataSet.getEntryForIndex(i);
                if (dataPoint.getHigh() > highestSpeed) {
                    highestSpeed = dataPoint.getHigh();
                    highestSpeedIndex = i;
                }
            }

            // PB Display on Speed chart
            List<GpsWorkout> workouts = Instance.getInstance(this.getContext()).db.getGpsWorkouts();
            ArrayList<PersonalBestRecord> bests = PersonalBestRecord.personalBestsFromList(workouts);
            double max = -1;
            for (PersonalBestRecord record : bests) {
                try {
                    if (record.getWorkoutData().get(PersonalBestRecord.PBType.AverageSpeed).contains(PersonalBestRecord.PBPeriod.Overall)) {
                        max = record.getWorkout().avgSpeed;
                    }
                } catch (NullPointerException ignored) {
                }
            }

            if (!speedTitle.isSwapped()) {
                setupPBLegend(speedChart, max, property.getUnit(context, combinedData.getYMax()), "Speed");
            }

            // Create background line
            LineDataSet lineDataSet = StatsProvider.convertCandleToMeanLineData(candleDataSet);

            // Set Default Style to Line
            lineDataSet = DataSetStyles.applyBackgroundLineStyle(context, lineDataSet);

            // Change Highest Speed Circle to Red & Default for others
            if (highestSpeedIndex != -1) {
                List<Integer> circleColors = new ArrayList<>();
                for (int i = 0; i < lineDataSet.getEntryCount(); i++) {
                    if (i == highestSpeedIndex) {
                        circleColors.add(Color.RED);
                    } else {
                        circleColors.add(ContextCompat.getColor(context, R.color.colorPrimary));
                    }
                }
    ...
    private void updateDistanceChart(List<WorkoutType> workoutTypes) {
        ...
        try {
            if (distanceTitle.isSwapped()) {
                ...
                // Find the Longest Distance Data Point on Chart
                double longestDistance = 0;
                int longestDistIndex = -1;
                for (int i = 0; i < candleDataSet.getEntryCount(); i++) {
                    CandleEntry dataPoint = candleDataSet.getEntryForIndex(i);
                    if (dataPoint.getHigh() > longestDistance) {
                        longestDistance = dataPoint.getHigh();
                        longestDistIndex = i;
                    }
                }

                // Change Longest Distance Circle to Red & Default for others
                if (longestDistIndex != -1) {
                    List<Integer> circleColors = new ArrayList<>();
                    for (int i = 0; i < lineDataSet.getEntryCount(); i++) {
                        if (i == longestDistIndex) {
                            circleColors.add(Color.RED); // Red for the highest point
                        } else {
                            circleColors.add(ContextCompat.getColor(context, R.color.colorPrimary)); // Default color for other points
                        }
                    }
                    lineDataSet.setCircleColors(circleColors); // Apply the circle colors
                }
            ...
            // Update New Changes to the Chart
            ...
            setupPBLegend(distanceChart, ((double) combinedData.getYMax() / 1000), "km", type);
    ...
    private void updateDurationChart(List<WorkoutType> workoutTypes) {
        ...
        try {
            if (durationTitle.isSwapped()) {
                ...
                int longestDuratIndex = -1;
                for (int i = 0; i < candleDataSet.getEntryCount(); i++) {
                    CandleEntry dataPoint = candleDataSet.getEntryForIndex(i);
                    if (dataPoint.getHigh() > longestDuration) {
                        longestDuration = dataPoint.getHigh();
                        longestDuratIndex = i;
                    }
                }

                // Change Longest Duration Circle to Red & Default for others
                if (longestDuratIndex != -1) {
                    List<Integer> circleColors = new ArrayList<>();
                    for (int i = 0; i < lineDataSet.getEntryCount(); i++) {
                        if (i == longestDuratIndex) {
                            circleColors.add(Color.RED); // Red for the highest point
                        } else {
                            circleColors.add(ContextCompat.getColor(context, R.color.colorPrimary)); // Default color for other points
                        }
